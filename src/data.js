
export const items = [
    {
      id: "a",
      category: "BlackPink",
      username:"Silvia",
      title: "Blackpink Ladies",
      numberofLike: 80,
      GroupName: "BlackPink has 4 members",
      MemberNames: "Their Korean names are: Rosé 로제  ,Lisa 리사 ,Jennie 제니 ,and Jisoo 지수",
      Country:"They are the first music group and Korean female act to have five music videos accumulate one billion views each on YouTube; Their other accolades include the New Artist of the Year Award at the 31st Golden Disc Awards and the 26th Seoul Music. They have been acknowledged as one of the most powerful celebrities in South Korea by Forbes Korea (placing first in 2019, third in 2020, and second in 2021) and by former South Korean President Moon Jae-in as a global K-pop phenomenon helping spread K-pop content worldwide. In 2022, Blackpink was named Time's Entertainer of the Year for their contributions to music.",
      Picture:"https://lastfm.freetls.fastly.net/i/u/770x0/d8fa3e34c1dc4fd4b87bb2372697d544.jpg#d8fa3e34c1dc4fd4b87bb2372697d544"
    },
    {
      id: "w",
      category: "Kpop Boys",
      username:"Shreyas",
      title: "All about BZ Boys",
      numberofLike: 120,
      GroupName:"BZ Boys",
      MemberNames:"Their Korean names are: Bon 본 ,Taewoong 태웅 ,Hamin 그게 다야 ,Double D 더블디 and Seunghyn 승훈",
      KoreanGroupName:"청공소년",
      Country: "From South Korea",
      FandomName:"BZ Boys also called Blue Zone Boys is a 5-member boy group under Chrome Entertainment. The debuted on June 17th, 2019 ",
      Picture:"https://i0.wp.com/kstationtv.com/wp-content/uploads/2021/08/BZ-BOYS-2.jpg?fit=600%2C400&ssl=1"
    },
    {
      id: "c",
      category: "BlackPink",
      username:"Mo",
      title: "Lisa from BlackPink",
      numberofLike: 260,
      MemberNames:"Lisa is the lead rapper of BlackPink.",
      Country:"She can speak Thai, English, and Korean.",
      DateGroupFormed:"Lisa made her solo debut with her single album Lalisa in September 2021. The album sold over 736,000 copies in its release week in South Korea, making her the first female artist to do so. The music video for its lead single of the same name recorded 73.6 million views on YouTube in first 24 hours of its release, becoming the most-viewed music video in the first 24 hours on the platform by a solo artist. The song earned Lisa two Guinness World Records as well as the first MTV Video Music Award ever won by a K-pop soloist.",
      FandomName:"Lisa is a brand ambassador for Moonshot cosmetics.",
      // Picture:"https://www.nme.com/wp-content/uploads/2021/09/lisa-blackpink-money-video-yg-entertainment-200921.jpg"
      Picture: "images/lisa.jpg"
    },
    {
      id: "d",
      category: "BTS",
      username:"Mo",
      title: "How much do you know about BTS",
      numberofLike: 200,
      Picture:"https://www.hollywoodreporter.com/wp-content/uploads/2019/09/thr__bts_-_photographed_by__alexandra_gavillet-_splash_-_2019.jpg?w=1024",
      MemberNames:"Their names in Korean are: Jung Kook 정국 ,V ,Jin 진 ,SUGA 슈가 ,Jimin 지민 and RM ",
      GroupName:"They're also known as Bangtan Boys",
      FandomName:"As of 2022, according to the Circle Chart, BTS is the best-selling artist in South Korean history, having sold in excess of 30 million albums. Their studio album Map of the Soul: 7 (2020) is the best-selling album of all time in South Korea. They are the first non-English-speaking and Asian act to sell out concerts at Wembley Stadium and the Rose Bowl.",
      Country:"They co-write and produce most of their own material."
    },
    {
      id: "f",
      category: "Kpop Girls",
      username:"Michelle",
      title: "The BEST BlackPink girl",
      numberofLike: 150,
      Picture:"https://img.kpopmap.com/680x384/2020/09/jennie-pink-hair-blackpink-ice-cream-uhdpaper.com-4K-7.2620-wp.thumbnail.jpg",
      MemberNames:"Jennie Was born in South Korea, Her family moved her to FL. to become a lawyer but she didn't like it, so they moved her back home. she studied in New Zealand for 5 years before returning and joining the group BlackPink.",
      Country:"She was the 1st Korean celebrity to model for Boucheron Paris, a luxurious jewlery brand. In April 2019, Jennie became the first Korean solo artist to perform at Coachella. Her performance was included in The 10 Best Things We Saw at Coachella 2019 list by Billboard, which called it mind-blowing and stunning for fans and casual passersby.",
      GroupName:"Jennie loves to wear vintage 90's clothes, and is the other rapper in BlackPink."
    },
    {
      id: "e",
      category: "Kpop Boys",
      username:"Silvia",
      title: "D1CE",
      numberofLike: 60,
      Picture:"https://0.soompi.io/wp-content/uploads/2019/09/12003559/D1CE-1.jpg",
      GroupName: "D1CE 디원스",
      DateGroupFormed:"They are a South Korean boy band formed by Happy Face Entertainment in 2019. The group consists of five members.",
      MemberNames:"Their names in Korean are: Yonggeun 용근 ,Hyunsoo 현수 ,Yoojin 유준 ,Woodam 우담 and Jinyoung 진영",
      FandomName:"Don1y is their fan name.",
      Country:"From South Korea"
    },
    {
      id:"1",
      title:"BlackPink Info",
      GroupName:"BlackPink",
      MemberNames:["Jennie Kim,"," Lalisa Manobal,"," Jisoo,"," Rosé"],
      KoreanGroupName:"블랙핑크",
      DateGroupFormed:2016,
      Country:"From Seoul, South Korea",
      FandomName:"BLINK is their fan name",
      Instagram:"Instagram is blackpinkofficial",
      Top3Songs:["Top 3 songs: ","Ddu-DuDdu-Du,"," BOOMBAYAH,"," Whistle"],
      Picture:"https://6.viki.io/image/6442cf3279fe465588ff07ee441c28f6/dummy.jpeg?s=900x600&e=t"
      },
      {
      id:"2",
      title:"BTS Info",
      GroupName:"BTS",
      MemberNames:["Jung Kook,"," V,"," Jin,"," SUGA,"," Jimin,"," RM"],
      KoreanGroupName:"방탄소년단",
      DateGroupFormed:"2010",
      Country:"From Seoul, South Korea",
      FandomName:"ARMY is their fan name",
      Instagram:"Instagram is bts.bighitofficial",
      Top3Songs:["Top 3 songs: ","My Universe,"," Dynamite,"," I Need U."],
      Picture:"https://media.cnn.com/api/v1/images/stellar/prod/220614113423-bts-file-0403.jpg?c=original"
      },
      {
      id:"3",
      title:"Twice Info",
      GroupName:"Twice",
      MemberNames:["Member's names: ","Nayeon,"," Momo,"," Tzuyu,","Sana,"," Jeongyeon,"," Jihyo,"," Mina,"," Chaeyoung,"," Dahyun"],
      KoreanGroupName:"트와이스",
      DateGroupFormed:2015,
      Country:"From South Korea",
      FandomName:"Once is their fan name",
      Instagram:"Instagram is twicetagram",
      Top3Songs:["Top 3 songs: ","Feel Special,"," Fancy,"," Cheer Up"],
      Picture:"https://people.com/thmb/vKeU9x0JeCmBGBGQZ2dfXCE7_dU=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():focal(749x0:751x2)/twice86-5e1a7532b1244f10acfd8f1921e2fc08.jpg"
      },
      {
      id:"4",
      title:"iKon Info",
      GroupName:"iKon",
      MemberNames:["Jinhwan,"," Bobby,"," Kim Donghyuk,"," KooJun-hoe,"," Song Yunhyeong,"," JungChanwoo"],
      KoreanGroupName:"아이콘",
      DateGroupFormed:2015,
      Country:"From Seoul, South Korea",
      FandomName:"IKONIC is their fan name",
      Instagram:"Instagram is withikonic",
      Top3Songs:["Top 3 songs: ","Airplane,"," My Type,"," Apology"],
      Picture:"https://www.billboard.com/wp-content/uploads/media/iKON-2018-cr-YG-Entertainment-billboard-1548.jpg"
      },
      {
      id:"5",
      title:"RedVelvet Info",
      GroupName:"RedVelvet",
      MemberNames:["Irene,"," Joy,"," Yeri,"," Wendy,"," Seulgi"],
      KoreanGroupName:"레드벨벳",
      DateGroupFormed:2014,
      Country:"From Seoul, South Korea",
      FandomName:"ReVeluv is their fan name",
      Instagram:"Instagram is redvelvet.smtown",
      Top3Songs:["Top 3 songs: ","Psycho,"," FeelMyRhythm,"," BadBoy"],
      Picture:"https://6.viki.io/image/cbb7aa15b28043a1b6e776bbc9d42ada/dummy.jpeg?s=900x600&e=t"
      }, 
      {
      id:"6",
      title:"BigBang Info",
      GroupName:"BigBang",
      MemberNames:["T.O.P,"," Taeyang,"," G-Dragon,"," Daesung"],
      KoreanGroupName:"빅뱅",
      DateGroupFormed:2006,
      Country:"From Seoul, South Korea",
      FandomName:"V.I.P is their fan name",
      Instagram:"Instagram is bigbang_official",
      Top3Songs:["Top 3 songs: ","FXXKIT,"," HaruHaru,"," Loser"],
      Picture:"https://www.allkpop.com/upload/2021/08/content/190018/1629346688-20200609-bigbang.jpg"
      }
  ];
  
  export const openSpring = { type: "spring", stiffness: 200, damping:30 };
  